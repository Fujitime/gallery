...
<div class="lg:ml-64 p-5 mt-20">
    ...
    <div class="flex justify-between">
        <h2 class="text-2xl font-bold mb-4 mt-3">User List</h2>
    ...
    <div class="overflow-auto relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <td class="px-6 py-4">{{ $user->name ? $user->name : $user->username }}</td>
            <td class="px-6 py-4"> {{ $user->email }} </td>
                ...
                <button type="button" class="delete-button flex items-center space-x-2 text-red-500 hover:underline" data-user-id="{{ $user->id }}">
        ...
        <div class="dark:bg-gray-900 bg-gray-200 shadow dark:shadow-none p-5 rounded-lg">
            ...
            <div class="relative p-4 text-center bg-gray-100 dark:bg-gray-800 rounded-lg sm:p-5">
                ...
                <div class="flex justify-end">
                    <form id="deleteForm" method="POST" action="{{ route('users.destroy', '__user_id__') }}">
                        @csrf
                        @method('DELETE')
...
<script>
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            deleteForm.action = '{{ route("users.destroy", ":user_id") }}'.replace(':user_id', userId);
